...
| Unbounded tool recursion | Partially addressed for nested capabilities. `CapabilityRuntime` keeps private immutable ancestry per child, refuses repeated identities and applies a finite `1..32` runtime depth limit before child policy, dependencies or effects. Ordinary application Python recursion remains application-owned; streams and cross-app composition are not part of this boundary. |
