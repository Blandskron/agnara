...
| Automated destructive invocation, replay of non-idempotent operations | Partially addressed. ADR 0091 makes an explicit direct `Idempotency.YES` selector claim atomically before dependencies or handler work and reuses only a bound completed success. It re-evaluates policy and confirmation, rejects scope/principal mismatch, and fails closed on store errors. It neither authenticates callers nor adds HTTP/MCP selector fields; anonymous HTTP and unreviewed protocol metadata remain untrusted. |
