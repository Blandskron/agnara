...
| Execution identity | `IMPLEMENTED` | ADR 0087. Each `ExecutionContext` owns an opaque runtime-generated identity; `invoke_result`, `CapabilityStream` and lifecycle telemetry retain it while each actual handler attempt still receives a separate telemetry `invocation_id`. Caller metadata cannot choose it. It is absent from introspection and transports, and adds no store, replay or retry behavior. |
