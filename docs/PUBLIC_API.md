...
The current 1.x contract contains **166 stable exports across 13 modules**: 166
...
neither occurs in the current inventory. `internal` names are outside the manifest
...
## Canonical spelling and authority
...
`agnara.di` is the supported DI entry point; `agnara.core.di` is an
implementation namespace. `IntrospectionSnapshot.applications` and the
top-level JSON `applications` field name compiled applications.
`ApplicationDescriptor.apps` names bounded contexts within an application.

`ExecutionContext.principal` must be a `Principal`; a token, claims mapping or
host session is not accepted as authority. `principal`,
`confirmation_evidence` and `idempotency` are read-only after construction.
Supply them from the composition root that verified the caller. `state` and
`tracking_id` are mutable but grant no authority.
