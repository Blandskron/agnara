...
| **Streaming and large uploads** | An upload is bounded `bytes`. Streaming request bodies need the streaming model, which is I2 and belongs to `0.1.0a8` (ADR 0068, ADR 0081). |
...
at `0.1.0a8`, when I2 decides streaming: a streamed request body would replace
